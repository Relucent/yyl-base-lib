...
public class Mapx extends MapWrapper<String, Object> {
	...
	// ==============================ChainMethods========================================
	/**
	 * 设置键值对（该方法返回当前MAP对象，用于链式写法）
	 * @param key   键
	 * @param value 值对象
	 * @return 当前MAP对象
	 */
	public Mapx _put(String key, Object value) {
		put(key, value);
		return this;
	}
	...
	public <T> void forEach(Class<T> elementType, BiConsumer<String, T> action) {
		Objects.requireNonNull(elementType);
		Objects.requireNonNull(action);
		super.forEach((k, v) -> action.accept(k, ConvertUtil.convert(v, elementType)));
	}
