...
/**
 * Bean信息描述，为{@code java.beans.BeanInfo}和{@code java.beans.BeanDescriptor} 的替代方案<br>
 * 查找Getter和Setter方法时会：
 *
 * <pre>
 * 1. 忽略字段和方法名的大小写
...
public class BeanDesc implements Serializable {
    ...
    public BeanDesc(Class<?> beanClass) {
        ...
        for (Field field : FieldUtil.getFields(this.beanClass)) {
            ...
            if (!Modifier.isStatic(field.getModifiers()) && !FieldUtil.isOuterClassField(field)) {
                ...
                this.propDescMap.putIfAbsent(pd.getFieldName(), pd);
            }
        }
    }
    ...
    public PropDesc getProp(String fieldName) {
        return this.propDescMap.get(fieldName);
    }
    ...
    public Field getField(String fieldName) {
        final PropDesc desc = this.propDescMap.get(fieldName);
