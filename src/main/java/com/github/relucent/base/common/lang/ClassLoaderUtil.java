...
public class ClassLoaderUtil {
	 ...
	 * 包名分界符: '.'
	 */
	private static final char PACKAGE_SEPARATOR = CharConstant.DOT;
	/**
	 * 内部类分界符: '$'
	 */
	...
	/**
	 * 加载类，通过传入类的字符串，返回其对应的类，使用默认ClassLoader并初始化类（调用static模块内容和初始化static属性）<br>
	...
	/**
	 * 加载类，通过传入类的字符串，返回其对应的类，使用默认ClassLoader<br>
	 * 扩展{@link Class#forName(String, boolean, ClassLoader)}方法，支持以下几类类名的加载：
	 *
	 * <pre>
	 * 1、原始类型，例如：int
