...
public class CharSequenceUtil {
    ...
    /**
     * 返回检索子字符序列在字符序列第一次出现时的索引，从指定索引开始搜索。
     * @param cs 字符序列
     * @param searchChar 查找的字符序列
     * @param start 开始检索位置
    ...
    static int lastIndexOf(final CharSequence cs, final CharSequence searchChar, int start) {
        ...
        while (true) {
            ...
            if (checkLaterThan(cs, searchChar, len2, i)) {
                return i;
            }
            i--;
            if (i < 0) {
     ...
     * @return 是否匹配
     */
    private static boolean checkLaterThan(final CharSequence cs, final CharSequence searchChar, final int len2, final int start1) {
